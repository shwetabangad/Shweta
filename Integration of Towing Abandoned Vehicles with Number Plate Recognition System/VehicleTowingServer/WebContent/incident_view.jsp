	...
	<div id="content">
		...
		<div class="third last">
			...
			<table border="0" cellspacing="10" cellpadding="3">
				...
				%>
				<tbody>
				<form action="UpdateNoPlateServlet?id=<%=incidentBeanList.get(i).getIncidenteId()%>&lat=<%=lat%>&long=<%=longi%>" method="post">
					<tr>
					
						<td><%=date%></td>
						<td><img height="100" width="100" src="IncidentImageServlet?id=<%=id%>"/></td>
						<td><input type="text" name="noPlate" id="noPlate"
											value="<%=noPlate%>" size="10" /></td>
						<td align="center"><%=fine%></td>
						
						<td><input type="submit" name="noPlate" id="noPlate"
											value="Update" /></td>
						<td align="center"><%if(smsSent.equals("1")){%>
							<a>YES</a>
								<% }else
								{%>
									<a>NO</a>
						...
						<% if(status == 0)
						{%>
							<td align="center"><a href="PaymentServlet?id=<%=id%>">Pay</a></td>
